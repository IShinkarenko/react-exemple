  ...
  return (
    ...
    >
      <div className={classes.CSVImporter_ProgressDisplay}>
        {isComplete || error ? (
          <div className={classes.CSVImporter_ProgressDisplay__status} role="status" tabIndex={-1} ref={statusRef}>
            {error ? (
              'Could not import'
            ) : (
              <Alert severity="success" sx={{ mb: '24px' }}>
                <AlertTitle>Import In Progress...</AlertTitle>
                Depending on list size, it may take a few minutes to see the imported relationships appear. Clicking
                Finish will take you to the listing immediately while the import continues in the background. You can
                refresh or return to the Relationships Listing to see imported companies as they are imported.
              </Alert>
            )}
          </div>
        ) : (
          <div className={classes.CSVImporter_ProgressDisplay__status_pending} role="status">
            Importing...
          </div>
        )}

        {/* <div className={classes.CSVImporter_ProgressDisplay__count} role="text">
